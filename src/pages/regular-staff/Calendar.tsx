...
export default function RegularStaffCalendar() {
  ...
  const checkEditPermission = () => {
    ...
    // Only admin and executives can edit
    const isAdmin = user.role === 'admin';
    const isExecutive = user.position && (
      user.position.toLowerCase().includes('editor-in-chief') ||
      user.position.toLowerCase().includes('associate editor') ||
      user.position.toLowerCase().includes('managing editor') ||
      user.position.toLowerCase().includes('executive secretary')
    );
  ...
  const loadAssignments = () => {
    ...
    // Filter by current user for regular staff
              ...
              <div className="md:hidden divide-y divide-gray-200">
                {currentItems.length === 0 ? (
                  ...
                  currentItems.map((assignment) => {
                    ...
                    return (
                      <div key={assignment.id} className="p-4 bg-white">
