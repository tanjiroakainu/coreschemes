...
export default function Coverage() {
  ...
  const handleSaveNote = (notes: string) => {
    ...
    saveScheduleNote({
      stafferId: selectedMember.stafferId,
      day: selectedDay,
      ...
      addedByPosition: currentUser.position || '',
    });
  ...
  const handleDeleteNote = () => {
    if (selectedNote) {
      deleteScheduleNote(selectedNote.id);
      loadScheduleNotes();
    }
  };
  ...
  return (
    <div className="px-4 md:px-10 py-6 space-y-6">
      <div className="flex items-center justify-between">
        <h1 className="text-2xl font-bold text-amber-600">My Team</h1>
      ...
      {selectedMember && (
        <div className="space-y-4">
          <h2 className="text-xl font-bold text-center">Class Schedule & Availability</h2>
          {selectedMember && (
                  ...
                  <tr key={timeSlot} className="hover:bg-gray-50">
                    ...
                    {DAYS.map((day) => {
                      ...
                      return (
                        <td
                          key={day}
                          className="px-2 py-2 text-xs text-center border border-gray-100 min-w-[120px] cursor-pointer hover:bg-amber-50 transition-colors"
      ...
      {teamMembers.length === 0 && (
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
          <p className="text-gray-500 text-center">
            No team members. Add members in "My Team" to view their schedules.
          </p>
        </div>
      )}
